...
# NB: DHW is a sensor/switch pair as there is no BOILER entity template
    _LOGGER.info("Started: setup_platform(), SWITCH")
...
# Collect any DHW zone as a (SWITCH component) device
    if tcsObjRef.hotwater:
        _LOGGER.info(
            "DHW SWITCH object: Found, zoneId(dhwId): %s, type: %s",
            tcsObjRef.hotwater.zoneId,
            tcsObjRef.hotwater.zone_type
        )

        slave = evoDhwSwitchDevice(hass, ec_api, tcsObjRef.hotwater)
        add_devices([slave], False)

    else:
        _LOGGER.info("DHW SWITCH object: None found")

    
    
    _LOGGER.info("Finished: setup_platform(), SWITCH")
    return True
